...
const AppointmentsPage = () => {
  ...
  // Listen for postMessage from the payment iframe
  useEffect(() => {
    const handlePaymentMessage = (event) => {
      // Replace '*' with the expected origin (e.g., 'http://197.138.207.30') for security
      if (event.origin !== 'http://197.138.207.30') return; // Adjust origin as needed

      const { type, status, receiptNumber, message } = event.data;
      ...
      if (type === 'paymentResult') {
        if (status === 'success' && receiptNumber) {
          ...
          notify(message || 'Payment failed. Please try again.', 'error');
          setWebViewVisible(false);
        }
      }
    };
  ...
  // Fetch appointments when tab changes to 'upcoming' or 'past'
  useEffect(() => {
  ...
  useEffect(() => {
    if (rescheduleOpen && newDate && rescheduleAppointmentId) {
      ...
      const doctorId = appointment
        ? isDoctors.find((doc) => doc.name === appointment.doctor)?.id
        : null;
      if (doctorId) {
        ...
        >
          {loading ? (
            ...
            <iframe
              ...
              title="M-Pesa Payment"
              onError={() => {
                notify('Failed to load payment page. Please try again or contact support.', 'error');
        ...
        <div className="m-0">
          <div className="grid grid-cols-3 gap-2">
            {['book', 'upcoming', 'past'].map((tabName) => (
              <button
                key={tabName}
                className={`py-2 shadow-md border rounded-t-md ${
                  tab === tabName ? 'bg-blue-800 text-white font-semibold shadow-md transation-all duration-600' : ''
                }`}
          ...
          {tab === 'book' && (
            <div className="bg-white border rounded-b-lg p-4 shadow">
              {loading ? (
                <div className="text-center">
                  <IsLoader isFullScreen={false} size="6" text="Loading Confirm Appointment..." />
                </div>
              ) : showConfirm ? (
                <>
                  ...
                  <div className="space-y-6 grid md:grid-cols-2 lg:grid-cols-3 gap-6 border p-5 rounded">
                    <p>
                      <strong>Patient:</strong> {userData?.AccountHolderName || patientid}
                    </p>
                    <p>
                      <strong>Center:</strong>{' '}
                      {isDoctors.find((doc) => doc.id === selectedDoctor)?.centre || '—'}
                    </p>
                    <p>
                      <strong>Doctor:</strong>{' '}
                      {isDoctors.find((doc) => doc.id === selectedDoctor)?.name || '—'}
                    </p>
                    <p>
                ...
                <>
                  ...
                  <div className="space-y-6 grid md:grid-cols-2 gap-6">
                      ...
                      value={
                        selectedDoctor
                          ? {
                              ...
                              label: isDoctors.find((doc) => doc.id === selectedDoctor)?.name || '',
                            }
                          : null
                      }
                    ...
                    <CustomSelect
                      ...
                      options={timeSlots}
                      value={
                        selectedSlot ? { value: selectedSlot.value, label: selectedSlot.label } : null
                      }
          ...
          {tab === 'upcoming' && (
            <div className="bg-white border rounded-b-lg p-4 shadow">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <CustomSelect
                      ...
                      value={
                        newSlot
                          ? { value: `${newSlot.fromTime}-${newSlot.toTime}`, label: `${newSlot.fromTime} - ${newSlot.toTime}` }
                          : null
                      }
                      onChange={(selectedOption) => {
              ...
              {loading ? (
                <div className="text-center">
                  <IsLoader isFullScreen={false} size="6" text="Upcoming Doctor Appointment..." />
                </div>
          ...
          {tab === 'past' && (
            <div className="bg-white border rounded-b-lg p-4 shadow">
              ...
              {loading ? (
                <div className="text-center">
                  <IsLoader isFullScreen={false} size="6" text="Past Doctor Appointment..." />
                </div>
                        ...
                        <div>
                          ...
                          <div
                            className={`text-xs font-extrabold ${
                              app.cancel === 1 ? 'text-red-600' : 'text-green-900'
                            }`}
                          >
